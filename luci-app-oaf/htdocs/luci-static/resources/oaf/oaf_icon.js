(function(window, $) {
    ...
    function createLetterIcon(name, options) {
        ...
        return $('<span>')
            ...
            .css({width: options.size || '20px', height: options.size || '20px', borderRadius: options.radius || '5px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: hashColor(name), color: '#fff', fontSize: options.fontSize || '11px', fontWeight: '700', flexShrink: 0, lineHeight: 1})
    ...
    function createAppIcon(appId, name, resourceBase, options) {
        ...
        if (id && !iconDisabled && iconStatusCache[id] === 'loaded') {
            return $('<img>').attr({src: src, alt: appName}).css({width: options.size || '20px', height: options.size || '20px', borderRadius: options.radius || '5px', objectFit: options.objectFit || 'cover', display: 'block', flexShrink: 0});
        }
        ...
        if (id && !iconDisabled && iconStatusCache[id] !== 'failed') {
            ...
            loader.onload = function() {
                ...
                $icon.replaceWith($('<img>').attr({src: src, alt: appName}).css({width: options.size || '20px', height: options.size || '20px', borderRadius: options.radius || '5px', objectFit: options.objectFit || 'cover', display: 'block', flexShrink: 0, border: 'none', boxShadow: 'none'}));
            };
            loader.onerror = function() { iconStatusCache[id] = 'failed'; };
            loader.src = src;
        }
        return $icon;
    }

    window.OAFIcon = {colors: iconColors, hashColor: hashColor, appIconSrc: appIconSrc, createLetterIcon: createLetterIcon, createAppIcon: createAppIcon};

    $(function() {
        if (!/\/app_filter\/rules\/?$/.test(window.location.pathname || '')) return;

        var ACTION_DEFAULT = 'block';
        var actions = {};
        var originalPost = $.post;
        var tableUpdating = false;
        var api = function(name) {
            if (window.L && L.url) return L.url('admin/services/oaf/api/app_filter/' + name);
        ...
        function normalizeRate(value) {
            ...
            return isNaN(n) || n < 0 || n > 10000000 ? 0 : n;
        }
        ...
        function ensureStyles() {
            ...
            $('head').append('<style id="oaf-rule-action-styles">#oaf-rule-action-group{border-top:1px solid var(--border-color-low,#d1d5db);padding-top:12px;margin-top:8px}#oaf-rule-action-group .oaf-rate-row{display:flex;gap:12px;flex-wrap:wrap;margin-top:8px}#oaf-rule-action-group .oaf-rate-field{display:flex;flex-direction:column;gap:4px;min-width:180px}#oaf-rule-action-group .oaf-rate-field input{max-width:180px}.oaf-rule-action-cell{white-space:nowrap}</style>');
        }
        ...
        function ensureFormFields() {
            ...
            var $hint = $('<div>').css({marginTop:'6px',color:'#777',fontSize:'12px'}).text('正常不执行限制；禁止沿用原有时间限制；限速仅对当前规则匹配的应用、用户和时间生效。');
            ...
            $anchor.after($group);
            function refreshRates() { $rates.toggle($('#oaf-rule-action').val() === 'limit'); }
        ...
        function applyActionToForm(ruleId) {
            ...
            setTimeout(function() { $('#rule-enabled').prop('checked', item.enabled !== 0); }, 0);
        }
        ...
        function loadActions(callback) {
            $.get(api('get_rule_actions'), function(resp) {
                ...
                refreshActionColumn();
            }).fail(function() { actions = {}; if (callback) callback(); });
        }

        function saveAction(ruleId, action, enabled, upload, download, callback) {
            ruleId = parseInt(ruleId,10);
            if (!ruleId) { if (callback) callback(false); return; }
            $.post(api('set_rule_action'), {data:JSON.stringify({rule_id:ruleId,action:normalizeAction(action),enabled:enabled?1:0,upload_kbps:normalizeRate(upload),download_kbps:normalizeRate(download)})}, function(resp) {
                var ok = !!(resp && resp.code === 0);
                if (ok) actions[String(ruleId)] = {enabled:enabled?1:0,action:normalizeAction(action),upload_kbps:normalizeRate(upload),download_kbps:normalizeRate(download)};
                if (callback) callback(ok);
                setTimeout(refreshActionColumn, 100);
            }).fail(function() { if (callback) callback(false); });
        }

        function deleteAction(ruleId) {
            $.post(api('del_rule_action'), {rule_id:parseInt(ruleId,10)}, function() { delete actions[String(ruleId)]; setTimeout(refreshActionColumn,100); });
        }

        function findNewRuleId(ruleData, callback) {
            $.get(api('get_filter_rules'), function(resp) {
                var list = (resp && resp.data && resp.data.list) || [];
                var candidates = list.filter(function(rule) { return String(rule.name || '') === String(ruleData.name || '') && parseInt(rule.mode || 1,10) === parseInt(ruleData.mode || 1,10); });
        ...
        function refreshActionColumn() {
            ...
            try {
                ...
                if ($head.length && !$head.find('.oaf-action-head').length) {
                    var statusIndex = -1;
                    $head.children('th').each(function(i) { if ($(this).text().trim() === 'Status') statusIndex = i; });
                    var $th = $('<th>').addClass('oaf-action-head').text('动作');
                    if (statusIndex >= 0) $head.children('th').eq(statusIndex).before($th); else $head.append($th);
                }
                $rows.each(function() {
                    var $row = $(this), $edit = $row.find('.btn-edit').first();
                    if (!$edit.length) return;
                    var ruleId = $edit.data('rule-id');
                    var item = actions[String(ruleId)] || {enabled:1,action:ACTION_DEFAULT,upload_kbps:0,download_kbps:0};
                    var text = actionLabel(item.action);
                    if (normalizeAction(item.action) === 'limit') text += ' ' + normalizeRate(item.upload_kbps) + '/' + normalizeRate(item.download_kbps) + ' Kbit/s';
                    var $cell = $row.find('.oaf-action-cell');
                    if (!$cell.length) $cell = $('<td>').addClass('oaf-action-cell');
                    if ($cell.text() !== text) $cell.text(text);
                    var statusCell = $row.children('td').filter(function() { var t=$(this).text().trim(); return t === 'Enabled' || t === 'Disabled' || t === '已启用' || t === '已禁用'; }).first();
                    if (statusCell.length && !$cell.parent().is(statusCell.parent())) statusCell.before($cell);
                    else if (!$cell.parent().is($row)) $row.append($cell);
                });
            } finally { tableUpdating = false; }
        }
        ...
        if (observerTarget && window.MutationObserver) {
            new MutationObserver(function() { if (!tableUpdating) setTimeout(refreshActionColumn, 0); }).observe(observerTarget, {childList:true,subtree:true});
        }

        document.addEventListener('click', function(event) {
            var $target = $(event.target);
            if ($target.closest('#btn-add-rule').length) { setTimeout(resetActionForm,0); return; }
            var $edit = $target.closest('.btn-edit');
            if ($edit.length) { var editId=$edit.data('rule-id'); setTimeout(function(){applyActionToForm(editId);},20); }
        }, false);

        $.post = function(url, data, success, dataType) {
            var urlText = String(url);
            var isRuleWrite = /\/api\/app_filter\/(add|update|delete)_filter_rule(?:$|[?#])/.test(urlText);
            if (!isRuleWrite) return originalPost.apply($, arguments);

            var parsed = null;
            if (data && typeof data.data === 'string') { try { parsed = JSON.parse(data.data); } catch (e) { parsed = null; } }
            ...
            var wrappedSuccess = function(resp) {
                ...
                if (/\/delete_filter_rule(?:$|[?#])/.test(urlText)) {
                ...
                var persist = function(id) { if (id) saveAction(id, action, enabledWanted, upload, download); };
                if (ruleId) persist(ruleId); else findNewRuleId(parsed || {}, persist);
                setTimeout(refreshActionColumn, 500);
            };
            return originalPost.apply($, [url,data,wrappedSuccess,dataType]);
        };
    });
